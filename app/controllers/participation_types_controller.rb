class ParticipationTypesController < ApplicationController
  ...
  def new
    @participation_type = ParticipationType.new
  ...
  def create
    @participation_type = ParticipationType.new(params[:participation_type])

    respond_to do |format|
      if @participation_type.save
        format.html { redirect_to @participation_type, notice: 'Participation type was successfully created.' }
